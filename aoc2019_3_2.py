...
def buildPath(inputArray, start=(0,0)):
	...
	for i in inputArray:
		direction = i[0]
		count = int(i[1:])
		#print("Using direction " + direction + " and count " + str(count))
		if direction == 'R':
			coordIndex = 0
			increment = 1
		elif direction == 'L':
		...
		for n in range (1, count + 1):
			if coordIndex == 0:
				newCoord = (currCoord[0] + increment, currCoord[1])
			elif coordIndex == 1:
				newCoord = (currCoord[0], currCoord[1] + increment)
			currCoord = newCoord
			pathArray.append(currCoord)
			steps += 1
			existingSteps = pathDict.get(currCoord, 0)
			if existingSteps == 0:
				pathDict[currCoord] = steps
	#print(pathArray)
	#print(pathDict)
	return (pathArray, pathDict)


def overlaps(path1, path2):
	overlap = []
	for item in path1.keys():
		#print(item)
		if item in path2:
			overlap.append(item)
	#return filter(in path2, path1.keys())
	return overlap

def getMinManhattanDist(coords, start=(0,0)):
	minDist = -1
	minCoord = start
	for coord in coords:
		currDist = calcManhattanDist(start, coord)
		if minDist <= 0 or currDist < minDist:
			minDist = currDist
			minCoord = coord
	return minDist

def getMinSteps(path1, path2, coords, start=(0,0)):
	minSteps = -1
	minCoord = start
	for coord in coords:
		coordSteps = path1.get(coord) + path2.get(coord)
		#print(coordSteps)
		if coordSteps < minSteps or minSteps <= 0:
			minSteps = coordSteps
	return minSteps

def closestWireIntersection():
	global WIRE_PATH_1
	global WIRE_PATH_2
	pathTuple1 = buildPath(WIRE_PATH_1)
	pathArray1 = pathTuple1[0]
	pathDict1 = pathTuple1[1]
	pathTuple2 = buildPath(WIRE_PATH_2)
	pathArray2 = pathTuple2[0]
	pathDict2 = pathTuple2[1]

	overlappers = overlaps(pathDict1, pathDict2)
	#print(overlappers)

	#minDist = getMinManhattanDist(overlappers)
	minDist = getMinSteps(pathDict1, pathDict2, overlappers)

	return minDist

print(closestWireIntersection())
